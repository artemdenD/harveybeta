...
public class NoSlow extends Module {
    ...
    public final Setting<Float> spookySlowFactor = new Setting<>("SpookySlowFactor", 0.5f, 0.1f, 1f);
    public final Setting<Integer> spookyDelay = new Setting<>("SpookyDelay", 5, 0, 20);

    private boolean returnSneak;
    private int spookyDelayTicks = 0;
    ...
    public void onUpdate() {
        ...
        if (mc.player.isUsingItem() && !mc.player.isRiding() && !mc.player.isFallFlying()) {
            switch (mode.getValue()) {
                ...
                case SpookyTime -> {
                    spookyDelayTicks++;
                    if (spookyDelayTicks < spookyDelay.getValue()) {
                        mc.player.input.movementForward *= spookySlowFactor.getValue();
                        mc.player.input.movementSideways *= spookySlowFactor.getValue();
                        return;
                    }
                    if (mc.player.getActiveHand() == Hand.OFF_HAND) {
                        sendPacket(new UpdateSelectedSlotC2SPacket(mc.player.getInventory().selectedSlot % 8 + 1));
        ...
        } else if (mode.getValue() == Mode.SpookyTime) {
            spookyDelayTicks = 0;
        }
    }
    ...
    public enum Mode {
        NCP, StrictNCP, Matrix, Grim, MusteryGrief, GrimNew, Matrix2, LFCraft, Matrix3, SpookyTime
    }
}
